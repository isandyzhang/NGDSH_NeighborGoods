...
import { reviewApi, type ReviewDetail, type ReviewStatus } from '@/features/reviews/api/reviewApi'
...
const ReviewBlock = ({ title, detail }: { title: string; detail: ReviewDetail }) => (
  <div className="space-y-2 rounded-xl border border-border bg-surface-2 px-3 py-3">
    <p className="text-sm font-semibold text-text-main">{title}</p>
    <p className="text-base text-text-subtle">評分：{detail.rating} / 5</p>
    <p className="text-base text-text-main">{detail.content || '（未填寫文字評價）'}</p>
    <p className="text-xs text-text-muted">{new Date(detail.createdAt).toLocaleString('zh-TW')}</p>
  </div>
)
...
export const CreateReviewPage = () => {
  ...
  const canSubmit = useMemo(() => Boolean(status?.viewerCanReview && !status.viewerReviewed), [status?.viewerCanReview, status?.viewerReviewed])
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus((current) => {
        ...
        const viewerIsBuyer = current.viewerIsBuyer
        return {
          ...current,
          viewerCanReview: false,
          viewerReviewed: true,
          viewerReviewBlockReason: null,
          viewerReview: review,
          canReview: false,
          reviewed: true,
          reason: '你已完成評價',
          review,
          ...(viewerIsBuyer
            ? { buyerCanReview: false, buyerReviewed: true, buyerReview: review, buyerReviewBlockReason: null }
            : { sellerCanReview: false, sellerReviewed: true, sellerReview: review, sellerReviewBlockReason: null }),
        }
      })
  ...
  const otherPartyReview = status
    ? status.viewerIsBuyer
      ? status.sellerReview
      : status.buyerReview
    : null
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-6 md:py-8">
      <section className="mb-8 space-y-3 text-center">
        ...
        <p className="text-lg text-text-subtle">完成交易後，買賣雙方可留下體驗回饋，幫助社群建立信任。</p>
      </section>
      ...
      {!loading && status ? (
        <div className="space-y-4">
          {otherPartyReview ? (
            <Card>
              <p className="mb-3 text-base font-semibold text-text-main">對方已留的評價</p>
              <ReviewBlock
                title={status.viewerIsBuyer ? '賣家給你的評價' : '買家給你的評價'}
                detail={otherPartyReview}
              />
            </Card>
          ) : null}

          <Card>
            {status.viewerReviewed && status.viewerReview ? (
              <div className="space-y-3">
                <p className="text-lg font-semibold text-text-main">你已完成此交易評價</p>
                <p className="text-base text-text-subtle">評分：{status.viewerReview.rating} / 5</p>
                <p className="rounded-xl border border-border bg-surface-2 px-3 py-3 text-base text-text-main">
                  {status.viewerReview.content || '（未填寫文字評價）'}
                </p>
                <p className="text-sm text-text-muted">
                  提交時間：{new Date(status.viewerReview.createdAt).toLocaleString('zh-TW')}
                </p>
              </div>
            ) : status.viewerCanReview ? (
              ...
              <EmptyState title="目前尚不可評價" description={status.viewerReviewBlockReason ?? '此交易尚未符合評價條件。'} />
            )}
          </Card>
        </div>
      ) : null}
